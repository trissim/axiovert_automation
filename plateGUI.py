...
class PlatePanel(wx.Panel):
    ...
    def hit_test(self, mx, my):
        size = self.size / 2
        for (row, col), (x, y) in self.coords.items():
            d = ((x - mx) ** 2 + (y - my) ** 2) ** 0.5
            if d < size:
                return (row, col)
        return None
...
def main(ctrl):
    def on_move_to(row, col):
        ...
        print(ctrl.plate.get_pos_ij(row,col))
        ctrl.move_to_well_ij(row, col)
    def on_move_to_and_scan(row, col):
        #Add move + snap
        print('on_move_to_and_scan', row, col)
    app = wx.App()
    model = PlateModel(ctrl.plate.rows, ctrl.plate.columns) 
    if model == None:
    ...
    if dialog.ShowModal() == wx.ID_OK:
        for key, label in model.labels.items():
            ...
            siteDict[label] = model.get_names(key)
        #for row in range(model.rows):
            #for col in range(model.cols):
                #key = model.grid[row][col]
